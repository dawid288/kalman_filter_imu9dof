...
extern "C" void app_main(void) {
  ...
  for (size_t n = 1; n < total_N * 16; n++) {
    ...
    //   gyro_data(0, 0) =
    //       1 / pi * std::cos(-pi / 2 + pi / 2 * count * 2 / (total_N / 10));
    //   gyro_data(1, 0) =
    //       2 / pi * std::cos(-pi / 2 + pi / 2 * count * 2 / (total_N / 10));
    //   gyro_data(2, 0) =
    //       3 / pi * std::cos(-pi / 2 + pi / 2 * count * 2 / (total_N / 10));
    //   count++;
    // }
    ...
    // and accel_norm and magn_norm should be real measured accel and magn
    // values The dt in this case should be real time difference in seconds
    // between samples Fill the input control values with measured gyro values
    ...
    // Here accel_norm and magn_norm should be real measured accel and magn
    // values
  ...
  std::cout << "Difference between real and estimated errors : "
            << (gyro_err - estimated_error).t() << std::endl;

  std::cout << "Expected Euler angels (degree) : "
            << (180 / pi * ekf::quat2eul(ekf::rotm2quat(Rm).data)).t();
  std::cout << "Calculated Euler angels (degree) : "
            << (180 / pi * ekf::quat2eul(ekf13->X.data)).t() << std::endl;
}
